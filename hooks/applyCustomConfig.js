    ...
    function updateIosPch (targetFilePath, configItems) {
        _.each(configItems, function (item) {
            if (item.data.tag === "string") {
                item.data.text && fs.appendFileSync(targetFilePath, os.EOL + item.data.text.trim() + os.EOL, { encoding: 'utf-8' });
            } else if (item.data.tag === "array") {
                fs.appendFileSync(targetFilePath, os.EOL, { encoding: 'utf-8' });
                _.each(item.data.getchildren(), function (child) {
                    child.text && fs.appendFileSync(targetFilePath, child.text.trim() + os.EOL, { encoding: 'utf-8' });
                });
            }
        });
    }
